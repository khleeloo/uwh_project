...
images_dir = 'images';
input_dir = fullfile(images_dir, 'input');
listing = cat(1, dir(fullfile(input_dir, '*.png')), ...
    dir(fullfile(images_dir, '*.CR2')));
%listing = cat(1, dir(fullfile(images_dir, '*_input.NEF')), ...
    %dir(fullfile(images_dir, '*.CR2')));
...
for i_img = 1:length(listing)
    ...
    output_restored_dir = fullfile(images_dir, 'output_restored');
    imwrite(im2uint8(img_out), fullfile(output_restored_dir, [img_name, '_output_img.jpg']));
    output_depthmap_dir = fullfile(images_dir, 'output_depthmap');
    imwrite(im2uint8(trans_out), jetmap, fullfile(output_depthmap_dir, [img_name, '_output_trans.jpg']));
