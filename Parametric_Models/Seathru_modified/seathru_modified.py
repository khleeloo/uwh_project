...
def load_image_and_depth_map(img_fname, depths_fname, size_limit = 1024):
    ...
    #depthsinput=cv2.imread("LFT_4007_output_trans.jpg")//test

    # 5xxx * 3xxx
    depthsinput=cv2.imread(depths_fname)
    # RGB to Grayscale
    depths_grey=cv2.cvtColor(depthsinput, cv2.COLOR_RGB2GRAY)

    #rawpyimg=cv2.imread("LFT_4007_input.jpg")
    rawpyimg=cv2.imread(img_fname) 

    # show format of Raw image
    # cv2.imshow("inputImg",rawpyimg) 
    img = Image.fromarray(rawpyimg)

    # resize 1024 * 1024
    img.thumbnail((size_limit, size_limit), Image.ANTIALIAS)

    # resize depthmap as the same as the resized Raw image
    #depths = depths.resize(img.size, Image.ANTIALIAS)
    depths = cv2.resize(depths_grey,img.size)#constant ratio
   
    #return np.float32(img) / 255.0, np.array(depths)
    return np.float32(img) / 255.0, (depths) 
'''
...
if __name__ == '__main__':
    ...
    # Images
    base_path_filesInputImg ="..\\images\\input"
    base_path_filesDepthMaps="..\\images\\output_depthmap"
    base_path_filesOutput   ="..\\images\\output_seathru"


    filesInputImg  = os.listdir(base_path_filesInputImg)
    filesDepthMaps = os.listdir(base_path_filesDepthMaps)
    filesOutput    = base_path_filesOutput

    #print(len(filesInputImg))
    #print(len(filesDepthMaps))

    #print(type(filesInputImg))
    #print(type(filesDepthMaps))


    i = 0
    for pathImg in filesInputImg:
        
        print(i, pathImg, filesDepthMaps[i])
        args.image     = base_path_filesInputImg + "\\" + pathImg
        args.depth_map = base_path_filesDepthMaps + "\\" +filesDepthMaps[i]
        args.output    = filesOutput + "\\" + pathImg.rsplit('.',1)[0] + "_output" + ".jpg"
        print(i, args.image, args.depth_map,args.output)
        print(i, "\n"+ args.image, "\n"+ args.depth_map, "\n"+ args.output)
        i = i + 1
        ...
        if args.preprocess_for_monodepth:
            ...
            plt.imsave(args.output, recovered)
            print(args.image, args.depth_map,args.output)
            print(i, 'Done.')
